...
/*
 * This simple demonstration program save to a file or send to the speakers sounds coming from an Urbi server.
 * The data written is the raw data as sent by the server (ers7 = 16000Hz stereo 16bit).
...
{
  char *usage = "usage:  urbisound robot milisecondtime  :plays sound recorded by the aibo to /dev/dsp\n" 
  ...
  if (argc >= 4) {
    ...
    int param;
    param=16;
    if (ioctl(f, SNDCTL_DSP_SAMPLESIZE, &param)==-1)  {fprintf(stderr,"failed to set sample size for %s\n", device); exit(1);}
    param=1;
    if (ioctl (f, SNDCTL_DSP_STEREO, &param)==-1) {fprintf(stderr,"failed to set stereo for %s\n", device); exit(1);}
    param=16000;
    if (ioctl (f, SNDCTL_DSP_SPEED, &param) == -1) {fprintf(stderr,"failed to set speed for %s\n", device); exit(1);}
#endif
  }
  ...
  client->send("loopsound:loop usound: micro.val ,   { wait(%d); stop loopsound; wait(1000); end:ping }, ", time);
  urbi::execute();
}
