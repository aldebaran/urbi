...
main(int argc, char* argv[])
{
  ...
  libport::OptionValue arg_exp("send SCRIPT to the server", "expression", 'e');
  libport::OptionValue arg_file("send the contents of FILE to the server", "file", 'f');
  libport::OptionValue arg_pfile("file containing the port to listen to", "port-file");
  libport::OptionFlag arg_banner("do not hide the server-sent banner", "banner", 'b');
  ...
  libport::cli_args_type remainings_args = opt_parser(libport::program_arguments());

  if (libport::opts::help.get())
    usage(opt_parser);
  banner = arg_banner.get();
  if (arg_exp.filled())
    data.push_back(new TextData(arg_exp.value()));
  if (arg_file.filled())
    data.push_back(new FileData(arg_file.value()));
