...
bool parseHeader(FILE *f) {
  ...
  for (int i=0;i<devCount;i++) {
	...
	int pos=0;
	do {
	  if ((device[pos++]=fgetc(f))==EOF) return false;
	}
	while (device[pos-1]);
	devices[i].name = strdup(device);
	if (fread(&devices[i].id,2,1,f)!=1) return false;
	int type;
	if ( (type=fgetc(f)) ==EOF) return false;
	devices[i].type=(UType)type;
  }
  return true;
}

void play(UClient * robot, FILE *f) {
  static int tick=0;
  static int ttime;
  static int prevtime=0; //timestamp of previous command
  UCommand uc;
  int starttime=0;
  ...
  while (fread(&uc,sizeof(uc),1,f)==1) {
	...
	UDev * dev=NULL;
	for (int i=0;i<devCount;i++)
	  if (devices[i].id==uc.id) {dev=&devices[i];break;}
	if (!dev) {
    ...
    if (robot) {
      if (lastCommandTime != uc.timestamp) {
        ...
        if (sleepstop-robot->getCurrentTime() > 500) { //queue no more than 500 ms in advance
...
int main(int argc, char * argv[]) {
  ...
  if ( (!strcmp(argv[1],"-")) || (!strcmp(argv[1],"+"))) {
	...
	robot=new UClient(argv[1]);
	robot->start();
	if (robot->error()) exit(4);
  }

  if (robot) robot->send("motoron;");
