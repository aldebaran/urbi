...
class BallTrackingHead {
 ...
 private:
  ...
  vector<PositionData> current_x, current_y; //joint value for the last few frames
...
UCallbackAction BallTrackingHead::getHead(bool pan, const UMessage &msg) { 
  if (msg.type != MESSAGE_DOUBLE)
    return URBI_CONTINUE;
 
  PositionData pd;
  pd.frame = msg.timestamp/32;
  pd.value = msg.doubleValue;
  if (pan) {
...
UCallbackAction BallTrackingHead::getImage(const UMessage &msg) { 
 ...
 static int frametime=0;
  ...
  double cx=0, cy=0;
  /*
  vector<PositionData>::iterator it = find(current_x.begin(), current_x.end(), msg.timestamp/32);
  if (it==current_x.end()) {
    return URBI_CONTINUE;
  }
  double cx = it->value;
  ...
  */
  ...
  if (msg.image.imageFormat == IMAGE_JPEG)
	convertJPEGtoYCrCb((const byte *)msg.image.data, msg.image.size, image, imgsize);
  else
	memcpy(image, msg.image.data, msg.image.width*msg.image.height*3);
  ...
  for (int i=0;i<msg.image.width;i++)
	for (int j=0;j<msg.image.height;j++) {
  ...
  mon->setImage((bits8*)image, msg.image.width*msg.image.height*3);
#endif 
...
BallTrackingHead::BallTrackingHead(const char * robotname) {
 robotI=new UClient(robotname);
  robotI->start();
  if (robotI->error()) urbi::exit(1);
  robotC=new UClient(robotname);
  robotC->start();
  if (robotC->error()) urbi::exit(1);
