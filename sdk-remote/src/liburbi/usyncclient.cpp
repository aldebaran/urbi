			  ...
			  int &width, int &height)
{
  ...
  if (f == 1  &&  format != urbi::IMAGE_JPEG ) {	//uncompress jpeg
    if (format == urbi::IMAGE_YCbCr)
  ...
  else if (format == urbi::IMAGE_RGB || format == urbi::IMAGE_PPM) {
    buffersize = std::min(m->value->binary->image.size, buffersize);
    if (m->value->binary->image.imageFormat == urbi::IMAGE_YCbCr)
...
USyncClient::syncGetNormalizedDevice(const char *device, double &val)
{
  ...
  if (m->type != MESSAGE_DATA || m->value->type != urbi::DATA_DOUBLE) {
    delete m;
    return 0;
  }
  val = (double)(*(m->value));
...
USyncClient::syncGetSound(const char * device, int duration, urbi::USound &sound)
{
  ...
  if (m->type != MESSAGE_DATA
      || m->value->type != urbi::DATA_BINARY
      || m->value->binary->type != urbi::BINARY_SOUND) {
    delete m;
    return 0;
  }
