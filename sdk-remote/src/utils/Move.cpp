...
int Move::initialize(UClient * client,bool uploadFiles, const char * configFile, bool enableInterrupt) {
  ...
  while (fgets(line,1024,cf)) {
	...
	if (uploadFiles) {
	  ...
	  if (!cmdf) {fprintf(stderr,"cannot open file %s, skipping\n",name); continue;}
  ...
  pwalk.maxSpeed=0;
  for (list<LoadedFile>::iterator it=walks.begin();it!=walks.end();it++) {
  ...
  pturn.maxSpeed=0;
  for (list<LoadedFile>::iterator it=turns.begin();it!=turns.end();it++) {
...
int Move::walk(float &distance, float precision, const char * tag) {
  ...
  int bestnummoves=-1;
  float currentval=0;
  list<LoadedFile>::iterator it=walks.begin();
  list<LoadedFile>::iterator it2=it;
	...
	for (int j=0;j<abs(bestmove[i]);j++) {
	  moving++;
	   if (moving<4)
	    ...
	    sequence.push_back((string)name);
	}
  }
  robot->send(command);
  distance=realmove;
...
int Move::turn(float &distance, float precision, const char * tag) {
  ...
  int bestnummoves=-1;
  float currentval=0;
  list<LoadedFile>::iterator it=turns.begin();
  list<LoadedFile>::iterator it2=it;
  for (int i=0;i<length/2;i++,it2++);
  // std::cerr << "values: ";
  for (int i=0;i<length/2;i++,it++,it2++) {
...
UCallbackAction Move::moveEnd(const UMessage &msg) {
  ...
  if (moving >0 && !sequence.empty()) {
    string s=sequence.front();
    sequence.pop_front();
    msg.client.send("exec(%s);",s.c_str());
  }
  if ( (moving<=0) && usertag[0]) robot->notifyCallbacks(UMessage(*robot, msg.timestamp,usertag,"***end move",NULL,0));
