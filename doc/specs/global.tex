...
\begin{itemize}
\item \refSlot{Global}{uobjects}, see below.
...
\item[evaluate] This \refObject{UVar} provides a synchronous interface
  ...
  yet, do not use \refSlot{Global}{evaluate} at all in \us).
...
\item \lstinline|getProperty(\var{slotName}, \var{propName})| This
  wrapper around \refSlot{Object}{getProperty} is actually a
  by-product of the existence of the \refSlot{Global}{evaluate}
  \refObject{UVar}.
...
\item \lstinline|isdef(\var{qualified-identifier})|\\
  ...
  compatibility.  See \refSlot{Object}{hasSlot} for a safer alternative.
\begin{urbiscript}
assert
{
  !isdef(a);
