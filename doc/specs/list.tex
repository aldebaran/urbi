...
assert_eq([0, 1, 2, 3, 4, 5].filter(function (v) {v % 2 == 1}), [1, 3, 5]);
\end{urbiscript}
...
\begin{urbiscript}[firstnumber=last]
assert_eq([0, 1, 2, 3].map(function (v) { v % 2 == 0}),
...
Remove all elements from the target that physically equals \var{that}.
...
Assign \var{value} to the element of the target at the given \var{index}.
...
\item \lstinline|'<'(\var{other})|\\
Return whether the target is inferior to the \var{other} list. A list is
