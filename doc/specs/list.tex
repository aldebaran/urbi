...
Return the target (\lst{list-asList}).
...
Empty the target (\lst{list-clear}).
...
Apply the given functional value on all members (\lst{list-each})
sequentially.
...
Return whether the target is empty (\lst{list-empty}).
...
Synonym for \lstinline|front| (\sect{std-list-front}).

\subsubsection{join}

Concatenate all members of the target to form a string, separating
them with the given separator (\lst{list-join}).
...
target is empty (\lst{list-popback}).
...
target is empty (\lst{list-popfront}).
...
second index excluded. An error if out of bounds (\lst{list-range}).
...
equal two by two (\lst{list-sameAs}).
...
Return the number of elements in the target (\lst{list-size}).
...
A synonym for \lstinline|push_back| (\sect{std-list-pushback}).
...
\subsubsection{[]}
A synonym for \lstinline|nth| (\sect{std-list-nth}).

\subsubsection{[]=}
A synonym for \lstinline|setNth| (\sect{std-list-setnth}).
