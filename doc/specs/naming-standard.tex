...
// Front laser of a robot equiped with multiple sonars:
...
braquets in the following table.
...
\item \lstinline|makeCompactNames| \\
  ...
  It automaticaly computes the short name of all the devices, and insert them as
...
Note that the \code{[]} function is using a mechanims to automaticaly look
for its argument as a slot of \code{Localization}. As a consequence, you cannot
pass a variable to this function, but only one of the constant \code{Localizer}.
To pass a variable, use the \code{get(loc)} or the \code{set(loc, value)}
function.

The following example illustrates a typical instanciation sequence:
