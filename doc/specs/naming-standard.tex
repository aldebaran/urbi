...
travelled by a point at the surface of the wheel.
...
\begin{slots}
  \slot{speed}%
  {%
    Translation speed in meters per second. Pointed to by the \code{val} slot.%
  }
\end{slots}
...
\begin{slots}
  \slot{remain}{Estimation of the remaining energy between 0 and 1.}
  \slot{capacity}{Storage capacity in Amp.Hour.}
  \slot{current}{Current current consumption in Amp.}
  \slot{voltage}{Current voltage in Volt.}
\end{slots}
