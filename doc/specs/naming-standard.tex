...
Layouts with identical components aranged in a circle can also use numeric
localization. The component with index 0 should be the uppermost, or frontmost
...
\begin{slots}
  ...
  \slot{turn(x)}%
  {%
    Turn right approximately \var{x} radians.  \var{x} can be a
...
\begin{supertabular}{|m{.115\linewidth}|m{.45\linewidth}|*{4}{m{.12\linewidth}|}}
  ...
  \component{wheel}{body}{}{RotationalMotor}{
    %%
...
\item \lstinline|makeCompactNames| \\
  This function must be called once on the root node (\code{robot}) after the
  hierarchy is completed.
  It automatically computes the short name of all the devices, and insert them as
  slots of the \code{Global} object.
...
% LocalWords:  verylightgray Gostai API UObject Localizer pown frontleft accel
% LocalWords:  insectoid functionalities IP Urbi PGain PID IGain DGain ufloat
% LocalWords:  LinearMotor RotationalMotor DistanceSensor TouchSensor gyrometer
% LocalWords:  AccelerationSensor GyroSensor TemperatureSensor angleMin VideoIn
% LocalWords:  angleMax distanceMin xfov yfov enum wb jpeg ImageConversion wav
% LocalWords:  AudioOut AudioIn BlobDetector TextToSpeech lang TTS voicexml Tri
% LocalWords:  VoiceXML SpeechRecognizer RGBLed animaloid localizer rangemin
% LocalWords:  rangemax eration cameraR cameraL speakerR speakerL microR microL
% LocalWords:  TextTo pre elbowR headYaw kneeL fingerR fingerLR urbi myCamera
% LocalWords:  addComponent addDevice makeCompactNames flatDump loc urbiscript
% LocalWords:  wheelRadius wheelDistance wheelL wheelR
