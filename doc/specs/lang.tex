...
\dfn[comment]{Comments} are used to document the code, they are ignored by the
...
% FIXME: New is no longer special.
\dfn[identifier]{Identifiers} in \us are composed of one or more alphanumeric or
...
\us supports classic arithmetic operators, with the classic semantics
...
\us supports classical comparison operators. See
...
method of the targeted object.\fixme{The examples are wrong}
...
\us features two different function calls: \dfn{strict} function
calls, effective arguments are evaluated before invoking the function,
and \dfn{lazy} function calls, arguments are passed as-is to the
...
Objects can contain any number of \dfn[slot]{slots}, every slot has a name and a
...
[00000000:hide] function (v) {
..  {
..    var '$switch' = v | var '$pattern' = Pattern . new("") | if ('$pattern' . match('$switch'))
..    {
..      echo("Empty string");
..    }
..    else
..    {
..      var '$pattern' = Pattern . new("foo") | if ('$pattern' . match('$switch'))
..      {
..        "bar";
..      }
..      else
...
The \lstinline|throw| keyword enables to throw exceptions, as shown in
\autoref{lst:throw}. Thrown exceptions will break the execution upward
until they are caught, or until they reach the toplevel --- as in
\Cxx.  Contrary to \Cxx, exceptions reaching the toplevel are
...
Exceptions are caught with the \lstinline|try|/\lstinline|catch|
construct. It consists of a first block (the \dfn[block!try-block]{try-block}),
from which we want to catch
exceptions, and one or more catch clauses to stop the exception
(\dfn[block!catch-block]{catch-blocks}). Each
catch clause defines a pattern against which the thrown exception is
matched. If no pattern is specified, the catch clause matches
systematically (equivalent to \lstinline|catch (...)| in \Cxx).
