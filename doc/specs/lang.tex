...
identifiers. The listing below illustrates this.
...
\begin{urbiscript}[firstnumber=last]
assert_eq(1 + if (true) 3 else 4, 4);
...
\begin{urbiscript}[firstnumber=last]
assert_eq({if (false) 10 else 20}, 20);
assert_eq({if (true) 10 else 20}, 10);

assert_eq({if (true) 10}, 10);

assert_eq({if (var x = 10) x + 2 else x - 2}, 12);
assert_eq({if (var x = 0)  x + 2 else x - 2}, -2);

if (var x = 123) x | x;
[00000005:error] !!! lookup failed: x
\end{urbiscript}
...
The \texttt{every} statement enables to execute a block of code
...
It exists in two flavors. The default flavor will execute the block in
the background every \texttt{period}. Several iteration can thus
overlap.
...
/*(*/sleep(2.6s);/*)*/
[00000000] *** Could you come over here?
[00001000] *** Could you come over here?
[00001500] *** There you are!
[00002000] *** Could you come over here?
[00002500] *** There you are!
\end{urbiscript}

On the other hand, the \texttt{every|} flavor will not let iterations
overlap. If an iterations duration exceeds the period, it will delay
all the following iterations. That is, the next iterations will start
immediately after the end of the current one, and next iterations will
occur normally every \texttt{period} from this point.
...
every|(1s)
{
  ...
  {
    too_long = false;
    echo("Please put me down");
    sleep(1.5s);
    echo("I don't hate you");
  }
  else
  {
    too_long = true;
...
/*(*/sleep(4.1s);/*)*/
[00000000] *** Please put me down
[00001500] *** I don't hate you
[00001500] *** No hard feelings
[00002500] *** Please put me down
[00004000] *** I don't hate you
[00004000] *** No hard feelings
