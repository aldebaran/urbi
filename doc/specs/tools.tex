...
\item[-q, --quiet] no output unless errors (implies \option{--batch})
...
provided:
\begin{description}
\item[\command{umake-shared}] Build a shared object to be later run
  using \command{urbi-launch} (autoref{sec:tools:urbi-launch}).  Same
  as \samp{umake --shared}.
\end{description}
