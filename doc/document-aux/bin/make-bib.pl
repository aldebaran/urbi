...
Report the C<@message> on C<STDERR> if C<$verbose> is higher that c<$level>.
...
Options:
  ...
  -q, --quiet        be less verbose.  Accumulates
  -v, --verbose      be more verbose.  Accumulates
EOF
  exit 0;
}
...
sub count_by_publication_type ()
{
  ...
  my @count =
    map (join (' & ',
	       map { sprintf "%2d", $_ }
	       count_by_publication_type_by_year ($_)),
	 @year);
  ...
  . '\hline'
  . "\n"
  . ' & \textbf{Chapitre} & & \textbf{Conférence} & \textbf{Conférence} & \textbf{Rapport de}\\\\'
  . "\n"
  . ' \textbf{Année} & \textbf{de livre} & \textbf{Journal} & \textbf{internationale} & \textbf{nationale} & \textbf{recherche}\\\\'
  . "\n"
  . '\hline'
  . "\n"
  . join ("\\\\\n", @count, '')
...
Getopt::Long::GetOptions
  ...
  or exit 1;
$bib = $ARGV[0];
unshift @year, reverse (1999 .. 2012);
unshift @year, "soumission"
  if $with_submitted;
verbose 0, "working on years: @year";
output ();
