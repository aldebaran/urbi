...
\fixme{Check the body (we don't use v) and the comment in
  valueAccessed}
\begin{cxx}
...
can set it to 0 to mean ``as fast as is useful''. \fixme{Adjust this
  to the lack of period in k2.}
...
Our device code can be written as follows: \fixme{There is a trailing
  UBinary bin in the last comment.}
\begin{cxx}
// Inherit from UObject.
...
one kernel period. If you do so, the \urbi code \lstinline{loop
  tag:micro.val,} will produce the expected result.
