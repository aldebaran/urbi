...
\lstinline{UBindFunction(classname, functionname)}. Once done, the function
can be called from \us.
...
\lstinline{UBindThreadedFunction(classname, functionname, lockMode)}.

The function code will be executed in a separate thread without breaking the
\us execution semantic.
...
LOCK\_NONE, LOCK\_FUNCTION, LOCK\_INSTANCE, LOCK\_CLASS or LOCK\_MODULE.
When set to LOCK\_NONE, no locking is performed. Otherwise, it limits
parallel execution to:

\begin{itemize}
\item One instance of the bound function for LOCK\_FUNCTION.
\item One bound function for each object instance for LOCK\_INSTANCE.
\item One bound function for the class for LOCK\_CLASS.
\item One bound function for the whole module (shared object) for LOCK\_MODULE.
\end{itemize}

There is however a restriction: you cannot mix multiple locking modes: for
instance a function bound with LOCK\_FUNCTION mode will not prevent another
function bound with LOCK\_INSTANCE from executing in parallel.
