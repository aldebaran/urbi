...
This section is meant for people who want to \emph{build} the \urbi
SDK.  If you just want to install a pre-built \urbi SDK, see
...
\item[Automake 1.11.1 or later] Note that if you have to install
  Automake by hand (as opposed to ``with your distro's system''), you
  have to tell its \command{aclocal} that it should also look at the
  files from the system's aclocal.  If \file{/usr/local/bin/aclocal}
  is the one you just installed, and \file{/usr/bin/aclocal} is the
  system's one, then run something like this:
...
At Gostai, you can use the script \file{common/install/install-boost}
...
\item[PDFLaTeX] TeX Live is the most common distro nowadays.
\begin{package}
...
\item[py-docutils] This is not a requirement, but it's better to have
  ...
  varies between distros.  It provides \command{rst2html}.  MacPorts:
  py26-docutils
...
\begin{description}
\item[Git 1.6.6 or higher]  There is direct support for submodules
  from \code{clone}.
...
\item[Old versions of Git] The following sequence expects that you
  have common/git (common being the Gostai repository) in your PATH.
...
\item[URBI\_IGNORE\_URBI\_U] Ignore failures (such as differences bw
  kernel revision and urbi.u revision) during the initialization.

\item[URBI\_LAUNCH] The path the urbi-launch that urbi.exe will exec.
...
There are several test suites that will be run if you run \samp{make
  check} (\samp{-j4} works on most machines).  To rerun only the tests
that failed, use \samp{make recheck}.
...
\item[executables.stamp] Updated when an executable is updated
  (urbi-launch, etc.).  Depends on libraries.stamp.

\item[urbi.stamp] When Urbi sources (\file{share/urbi/*.u}) are
  updated.
...
At random places we use \samp{+=} in \command{/bin/sh} scripts, which
\command{dash} does not support.  Please, use \command{bash} or
\command{zsh} instead of \command{dash} as \command{/bin/sh}.
